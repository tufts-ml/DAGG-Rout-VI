...
class GRANMixtureBernoulli(nn.Module):
    ...
    def __init__(self, config, max_nodes):
    ...
    def forward(self, input_dict):
        ...
        if not is_sampling:
            ...
            adj_loss,nll = mixture_bernoulli_loss(label, log_theta, log_alpha,
...
def mixture_bernoulli_loss(label, log_theta, log_alpha, adj_loss_func,
                           ...
                           sum_order_log_prob=False, return_neg_log_prob=True, reduction="none"):
    """
...
def evaluate_loss(args, model, data):
    gran = model['gran']
    nll = gran(data)
    return nll
...
def create_model(args, feature_map):
    ...
    gran_mix = GRANMixtureBernoulli(args, max_nodes).to(args.device)
...
def predict_graphs(eval_args):
    ...
    input_dict['batch_size'] = eval_args.count
    input_dict['num_nodes_pmf'] = num_nodes_pmf_train
